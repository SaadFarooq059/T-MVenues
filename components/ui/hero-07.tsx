...
export function Hero07({
  ...
  const taglineElement = tagline && (
    <p
      className={cn(
        'max-w-xs text-xs font-medium uppercase leading-relaxed tracking-[0.28em] text-gold sm:text-sm',
        vs.tagline,
      )}
    >
  ...
  const titleElement = title && (
    <h1
      className={cn(
        'text-balance font-serif font-normal tracking-tight text-ink',
        vs.title,
      )}
    >
  ...
  const descriptionElement = description && (
    <p
      className={cn(
        'max-w-xl leading-relaxed text-ink/65',
        vs.description,
      )}
    >
    ...
    <div className="relative w-full overflow-hidden">
      <div
        className={cn(
          ...
          '[mask-image:linear-gradient(to_bottom,black_80%,transparent_95%)]',
        )}
      >
        <div
          aria-hidden
          className="pointer-events-none absolute inset-0 z-10 bg-champagne/15 mix-blend-overlay"
        />
        ...
        <img
          ...
          alt={landscapeAlt}
          decoding="async"
          className="aspect-[2/1] w-full object-cover object-center outline outline-ink/10 sm:aspect-[9/4]"
        />
      </div>
    </div>
  )
